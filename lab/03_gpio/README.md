...
В нашем проекте будем по удерживанию кнопки будем зажигать светодиод, а при отпускании гасить.

Обратимся к [схеме отладочной платы BluePill](https://github.com/Rocket-B/STM32_labs/blob/main/doc/Blue_Pill.pdf), как мы видим, светодиод **D2** подключен к ножке stm **PC13**. Так как на плате BluePill нет кнопки, то будем использовать внешнюю кнопку, подключим одну ножку кнопки к GND а другую к **PC14**.
...
При нажатой кнопке будем зажигать светодиод, при отжатой соответственно его гасить.
...
```
  ...
  while (1)
  {
    ...
    /* USER CODE END WHILE */
```
Соберем программу на нашем компьютере нажав ![](./images/build.png), далее если никаких ошибок в коде нет, то в консоли внизу получаем сообщение ```Build Finished. 0 errors, 0 warnings.```

![](./images/console.png)

После этого можем залить код на наш микроконтроллер, для этого нажимаем кнопку Run ![](./images/Run.png) либо можем прошить и отладить через Debug ![](./images/Debug.png).

Если все сделали правильно, то должно получиться, что мы и ожидали: светодиод горит при нажатой кнопке и гаснет при отжатой.
